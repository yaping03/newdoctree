            ...
            if ".txt" in file:
                ...
                with open(file_path,"r",encoding="utf8")as f:
                    ...
                    for i in f:
                        ...
                        i = i.replace("\xa0"," ")
                        data = i.split("::")
                        if len(i.strip(" "))>6 and len(i.split(" "))==1 and i.strip(" ").startswith("第") and (0<i.strip("").find("条") <7):
                            tmp = i.split("条",maxsplit=1)
                            i = tmp[0]+"条"+" "+tmp[1]
                        elif len(i.strip(" "))>6 and len(i.split(" "))==1 and i.strip(" ").startswith("第") and (0<i.strip("").find("节") <7):
                            tmp = i.split("节", maxsplit=1)
                            i = tmp[0] +"节"+ " " + tmp[1]
                        elif len(i.strip(" "))>5 and len(i.split(" "))==1 and i.strip(" ").startswith("第") and (0<i.strip("").find("章") <7):
                        ...
                        if "第一" in chapter[0]:
                            ...
                            pass
                        if start == 0 and len(data)==1:
                            law["名称"] = data[0].strip("\ufeff").strip("《》").strip("\n")
                            start = 1
                        elif len(data) == 2:
                            law[data[0].strip("\ufeff").strip("《》")] = data[1].strip(" ").strip("\n")
                            ...
                            if "第" in chapter[0] and "章" in chapter[0]:
                                section_num = 0
                                id += 1
                                if len(chapter) == 3:
                                    title = chapter[1] + chapter[2].strip("\n")
                                else:
                                ...
                                section_num += 1
                                if len(chapter) == 3:
                                    section = chapter[1]+chapter[2].strip("\n")
                                else:
                ...
                for item in law["provision"]:
                    for k, v in item.items():
                        ...
                        if parent:
                            ...
                            parent = models.Title.objects.filter(law=obj, self_num=v["chapter_num"], level="章").first()
                            if parent:
                                new = models.Provision(law=obj,content=v["content"], serial_number=k, types="章条文", parent=parent,
                                                       num=v["num"])
                            else:
        ...
        with open(folder+"/中华人民共和国刑法（2015修正）.txt", "r", encoding="utf8")as f:
            ...
            if len(data) == 2:
                ...
                if "第"in chapter[0] and "编" in chapter[0]:
                    ...
                    section_num += 1
                    section = chapter[0]
                    law["section"].append({section: {"title": chapter[1].strip("\n"),"chapter_num": id, "level": "节","section_num":section_num,"ed_num":ed_num}})
                elif "第" in chapter[0] and "条" in chapter[0]:
                    num+=1
                    provision=chapter[0]
            ...
            for item in law["chapter"]:
                for k,v in item.items():
                    ...
                    if parent:
                        models.Title.objects.create(law=obj,name=v['title'],level="章",self_num=v["chapter_num"],parent_num=v["ed_num"],parent=parent)
                    else:
                        models.Title.objects.create(law=obj, name=v['title'], level="章", self_num=v["chapter_num"])
            for item in law["section"]:
                for k,v in item.items():
                    parent = models.Title.objects.filter(law=obj,self_num=v["chapter_num"],parent_num=v["ed_num"],level="章").first()
                    if parent:
                        models.Title.objects.create(law=obj,name=v["title"],level="节",parent=parent,self_num=v["section_num"],parent_num=v["chapter_num"])
                    else:
            ...
            for item in law["provision"]:
                for k,v in item.items():
                    parent_ed = models.Title.objects.filter(law=obj,self_num=v["chapter_num"],parent_num=v["ed_num"],level="章").first()
                    parent = models.Title.objects.filter(self_num=v["section_num"],parent_num=v["chapter_num"],level="节",parent=parent_ed).first()
                    if parent:
                        models.Provision.objects.create(law=obj,content=v["content"], serial_number=k, types="节条文", parent=parent,num=v["num"])
                    else:
                        parent = models.Title.objects.filter(law=obj,self_num=v["chapter_num"],parent_num=v["ed_num"],level="章").first()
                        if parent:
                            models.Provision.objects.create(law=obj,content=v["content"], serial_number=k, types="章条文", parent=parent,num=v["num"])
                        else:
                            parent = models.Title.objects.filter(law=obj, self_num=v["ed_num"], level="编").first()
                            if parent:
                                models.Provision.objects.create(law=obj,content=v["content"], serial_number=k, types="编条文", parent=parent,num=v["num"])
            ...
            #     section_num += 1
            #     section = chapter[0]
            #     law["section"].append({section: {"title": chapter[1].strip("\n"),"chapter_num": id, "level": "节","section_num":section_num}})
            # elif len(chapter) == 1 and chapter != ["\n"]:
            #     law["meta"] = i.strip("\n").strip("()")
            # else:
            #     pass
